...
public partial class MainWindow : FluentWindow
{
    ...
    private void OnViewerMessage(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        try
        {
            ...
            {
                Dispatcher.Invoke(() => UpdateModelPartsFromMessage(json));
            }
    ...
    private async void OnConvert(object sender, RoutedEventArgs e)
    {
        ...
        bool isWeapon = _vm.ExportMode == ViewModels.ExportMode.Weapon;
        var excludeMeshes = _vm.ModelParts.Where(p => !p.IsVisible).Select(p => p.OriginalName)
            .Concat(_vm.DeletedPartNames)
            .Distinct(System.StringComparer.Ordinal)
            .ToList();
        var req = BuildConvertRequest(_vm.PropName, excludeMeshes, _vm.IsPackMode && !isWeapon);

        var runner = new EngineRunner();
        var outcome = await runner.RunAsync(req, onLog: OnEngineLog);
    ...
    {
        bool isWeapon = _vm.ExportMode == ViewModels.ExportMode.Weapon;
        var weaponArch = _vm.WeaponArchetypeIndex switch
        {
            1 => EngineRunner.WeaponArchetype.Rifle,
            2 => EngineRunner.WeaponArchetype.Smg,
            3 => EngineRunner.WeaponArchetype.Shotgun,
            4 => EngineRunner.WeaponArchetype.Sniper,
            _ => EngineRunner.WeaponArchetype.Pistol,
        };
        string Vec3(double x, double y, double z)
        {
            var ci = System.Globalization.CultureInfo.InvariantCulture;
            return $"{x.ToString(ci)},{y.ToString(ci)},{z.ToString(ci)}";
        }
        ...
        return new EngineRunner.ConvertRequest(
            ...
            IncludeYtyp: _vm.IncludeYtyp && !isWeapon,
            ExtractTextures: _vm.ExtractTextures || isWeapon,
            ...
            Mode: isWeapon ? EngineRunner.ConvertMode.Weapon : EngineRunner.ConvertMode.Prop,
            WeaponArchetype: weaponArch,
            WeaponName: isWeapon ? _vm.WeaponName : null,
            WeaponSlot: isWeapon ? _vm.WeaponSlot : null,
            MuzzleOffset:   isWeapon ? Vec3(_vm.MuzzleX, _vm.MuzzleY, _vm.MuzzleZ) : null,
            GripOffset:     isWeapon ? Vec3(_vm.GripX,   _vm.GripY,   _vm.GripZ)   : null,
            MagazineOffset: isWeapon ? Vec3(_vm.MagX,    _vm.MagY,    _vm.MagZ)    : null,
            EjectOffset:    isWeapon ? Vec3(_vm.EjectX,  _vm.EjectY,  _vm.EjectZ)  : null,
            GenerateLods:   !isWeapon && _vm.GenerateLods,
    ...
    private async void OnSplitLayersToYdrs(object sender, RoutedEventArgs e)
    {
        ...
        if (_vm.ExportMode == ViewModels.ExportMode.Weapon)
        {
            AppDialog.Show(
                "Split-into-separate-YDRs only works in Prop mode. Weapons are single fragments.",
                "Split layers",
                System.Windows.MessageBoxButton.OK,
                System.Windows.MessageBoxImage.Information,
                this);
    ...
    private void OnFinalizePack(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            result = FiveOS.Services.PropPackBuilder.Build(session);
        }
        catch (Exception ex)
        {
    ...
    private string StageModelInSessionDir(string path)
    {
        ...
        // Some artists put textures in a sibling "textures/" folder beside
        // the model dir. Copy that too if present.
    ...
    /// fuzzy-matches each picked file to a part by filename stem and
    /// applies it as the part's diffuse via the same JS bridge the
    /// per-layer "Change textures..." menu uses. Useful when the source
    /// file ships without bundled textures (.obj is the common case).
    /// Preview only — same caveat as the per-layer path; not baked
    /// into the source file.</summary>
    private async void OnAddMissingTextures(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var path in dlg.FileNames)
        {
            ...
            // Fuzzy match: prefer an exact case-insensitive equal; fall
            // back to part name containing the stem or stem containing
            // the part name. Case-insensitive on both sides.
            var stemLower = stem.ToLowerInvariant();
            var match = parts.FirstOrDefault(p =>
                string.Equals(p.Name, stem, StringComparison.OrdinalIgnoreCase));
            if (match == null)
            {
            ...
            try
            {
                ...
                {
                    using var img = new ImageMagick.MagickImage(await File.ReadAllBytesAsync(path));
                ...
                var safeName = System.Text.Json.JsonSerializer.Serialize(match.OriginalName);
                var safeUrl  = System.Text.Json.JsonSerializer.Serialize(dataUrl);
                await Viewport.CoreWebView2.ExecuteScriptAsync(
                    $"window.setPartTexture && window.setPartTexture({safeName}, {safeUrl})");
                applied++;
            }
            catch
            {
        ...
        _vm.StatusText = skipped > 0
            ? $"Textures: {applied} applied, {skipped} skipped (no matching part or decode error)."
            : $"Textures: {applied} applied to {applied} part(s) — preview only, not baked.";
    }
    ...
    private async void OnLayerConfirmOptimizeMesh(object sender, RoutedEventArgs e)
    {
        ...
        _vm.StatusText =
            ...
            $" — wireframe enabled, press F or click the bottom-right toggle to turn off.";
    ...
    /// then live-swap the part's diffuse map in the viewer. Preview only —
    /// not persisted to the source file (yet).</summary>
    private async void OnLayerChangeTextures(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            // Hand the texture to JS as a data: URL so we don't have to
            // re-map the virtual host. PNG bytes work directly; other
            // formats get re-encoded to PNG for the live preview.
            byte[] bytes;
            var ext = Path.GetExtension(dlg.FileName).ToLowerInvariant();
            if (ext is ".png" or ".jpg" or ".jpeg" or ".bmp")
            {
                bytes = await File.ReadAllBytesAsync(dlg.FileName);
            }
            else
            {
                using var img = new ImageMagick.MagickImage(await File.ReadAllBytesAsync(dlg.FileName));
                img.Format = ImageMagick.MagickFormat.Png;
                bytes = img.ToByteArray();
            }
            var mime = ext switch
            ...
            var safeName = System.Text.Json.JsonSerializer.Serialize(part.OriginalName);
            var safeUrl  = System.Text.Json.JsonSerializer.Serialize(dataUrl);
            await Viewport.CoreWebView2.ExecuteScriptAsync(
                $"window.setPartTexture && window.setPartTexture({safeName}, {safeUrl})");

            _vm.StatusText = $"Swapped diffuse on '{part.Name}' (preview only — not saved to source).";
        }
        catch (Exception ex)
        {
    ...
    private void OnInnerToggleClick(object sender, RoutedEventArgs e)
    {
        ...
        bool destinationDiscards = view is "Weapon" or "Optimize";
