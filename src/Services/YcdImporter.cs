// Copyright (c) 2026 FiveOS. All rights reserved.
// https://github.com/w3bportal/FiveOS

using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System.Numerics;
using System.Reflection;
using System.Text.Json;
using CodeWalker.GameFiles;

namespace FiveOS.Services;

/// <summary>
/// Read .ycd binaries back into the editor's timeline. The writer side
/// (<see cref="YcdAnimationBuilder"/>) emits an XML clip that round-trips
/// through CodeWalker's resource framing; on the read side we lean on
/// the same library but pull tracks out via reflection rather than
/// hard-coding internal type members. CW's internal model has been a
/// moving target across versions — reflective access tolerates renames
/// and lets the importer keep working without a recompile.
///
/// Output is the same JSON payload <c>window.setKeyframes</c> accepts so
/// the host can simply forward it to the viewer.
/// </summary>
public static class YcdImporter
{
    /// <summary>Preview imports cap resolution so WebView2 isn't fed
    /// multi-megabyte per-frame JSON. Export re-samples from the baked
    /// clip at the user's chosen FPS — 30 Hz preview is plenty for scrub.</summary>
    private const int PreviewMaxFps = 30;
    private const int PreviewMaxFrames = 900;

    private static int ComputePreviewFrameCount(int sourceFrames, int sourceFps, float duration)
    {
        int fps = Math.Clamp(Math.Min(sourceFps, PreviewMaxFps), 1, 120);
        int frames = Math.Max(2, sourceFrames);
        if (duration > 0.001f)
            frames = Math.Min(frames, (int)Math.Ceiling(duration * fps) + 1);
        return Math.Clamp(frames, 2, PreviewMaxFrames);
    }

    /// <summary>Result of a successful import: clip name (from the YCD
    /// dictionary), bone tracks resolved to display names, plus the
    /// raw JSON payload ready to ship to the viewer.</summary>
    public record ImportResult(
        string ClipName,
        int KeyframeCount,
        int FrameCount,
        int Fps,
        double Duration,
        int MappedBones,
        int UnmappedTracks,
        string PayloadJson);

    /// <summary>Import the FIRST clip out of a YCD file. Most player
    /// emotes are single-clip; multi-clip handling can layer on top
    /// later (the picker UI is the only missing piece — the reader
    /// already returns the full clip list via <see cref="ListClips"/>).</summary>
    public static ImportResult ImportFirst(string path, IReadOnlyList<string> rigBoneNames)
        => ImportFirstBundled(path, rigBoneNames).Result;

    /// <summary>Load the file once, list clip names, and import the first
    /// clip — avoids parsing the same .ycd twice on large emotes.</summary>
    public static (List<string> Clips, ImportResult Result) ImportFirstBundled(
        string path, IReadOnlyList<string> rigBoneNames)
        => ImportFirstBundled(File.ReadAllBytes(path), Path.GetFileName(path), rigBoneNames);

    /// <summary>In-memory variant for RPF-extracted .ycd bytes.</summary>
    public static (List<string> Clips, ImportResult Result) ImportFirstBundled(
        byte[] data, string name, IReadOnlyList<string> rigBoneNames)
    {
        var ycd = LoadYcd(data, name);
        var clipDict = ycd.ClipDictionary
            ?? throw new InvalidDataException("YCD has no clip dictionary.");

        var clips = ListClipsFromDictionary(clipDict);
        var resolved = ResolveFirstClipAndAnimation(clipDict)
            ?? throw new InvalidDataException("YCD clip references an animation that isn't in the dictionary.");

        return (clips, BuildClip(clipDict, resolved.name, resolved.anim, resolved.source, rigBoneNames));
    }

    /// <summary>List clip names in the dictionary without importing —
    /// used by the host to drive a "Pick clip" dialog when the file
    /// holds more than one.</summary>
    public static List<string> ListClips(string path)
        => ListClips(File.ReadAllBytes(path), Path.GetFileName(path));

    /// <summary>In-memory variant for RPF-extracted .ycd bytes.</summary>
    public static List<string> ListClips(byte[] data, string name)
    {
        var ycd = LoadYcd(data, name);
        return ListClipsFromDictionary(ycd.ClipDictionary);
    }

    private static List<string> ListClipsFromDictionary(ClipDictionary? clipDict)
    {
        var list = new List<string>();
        if (clipDict is null) return list;

        // ClipMap is the authoritative name list; the Clips pointer array is
        // sparse (mostly null slots) and its enumerator throws anyway.
        // Use the same display names ImportNamed can resolve (ShortName /
        // basename) — MetaHash.ToString() alone often doesn't match Clip.Name.
        if (clipDict.ClipMap is { Count: > 0 })
        {
            var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var kv in clipDict.ClipMap)
            {
                var label = ClipListName(kv.Key, kv.Value);
                if (seen.Add(label))
                    list.Add(label);
            }
            return list;
        }

        var clips = ReadEnumerable(clipDict, "Clips");
        if (clips is null) return list;
        foreach (var c in clips)
        {
            var name = ReadString(c, "ShortName") ?? ReadString(c, "Name") ?? "(unnamed)";
            list.Add(ClipBasename(name) ?? name);
        }
        return list;
    }

    /// <summary>Clips that can resolve to a real Animation with bone
    /// sequences — use this for the library "working only" filter so
    /// broken ClipMap stubs (no wired anim) never appear.</summary>
    public static List<string> ListPlayableClips(byte[] data, string name)
    {
        var ycd = LoadYcd(data, name);
        var clipDict = ycd.ClipDictionary;
        var list = new List<string>();
        if (clipDict is null) return list;

        if (clipDict.ClipMap is { Count: > 0 })
        {
            var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            foreach (var kv in clipDict.ClipMap)
            {
                var label = ClipListName(kv.Key, kv.Value);
                if (!seen.Add(label)) continue;
                if (TryResolvePlayable(clipDict, kv.Key, kv.Value, label) is not null)
                    list.Add(label);
            }
            return list;
        }

        // Sparse Clips[] path — keep names that resolve.
        foreach (var label in ListClipsFromDictionary(clipDict))
        {
            try
            {
                // Cheap resolve probe via the same ImportNamed matching.
                var probe = ResolveNamedAnimation(clipDict, label);
                if (probe is not null && AnimationLooksPlayable(probe))
                    list.Add(label);
            }
            catch { /* skip */ }
        }
        return list;
    }

    /// <summary>True when the dictionary has at least one playable clip.</summary>
    public static bool HasPlayableClip(byte[] data, string name)
        => ListPlayableClips(data, name).Count > 0;

    /// <summary>Import a specific named clip — exposed for the multi-
    /// clip picker and the Animation Library.</summary>
    public static ImportResult ImportNamed(string path, string clipName, IReadOnlyList<string> rigBoneNames)
        => ImportNamed(File.ReadAllBytes(path), Path.GetFileName(path), clipName, rigBoneNames);

    /// <summary>In-memory variant for RPF-extracted .ycd bytes.</summary>
    public static ImportResult ImportNamed(
        byte[] data, string name, string clipName, IReadOnlyList<string> rigBoneNames)
    {
        var ycd = LoadYcd(data, name);
        var clipDict = ycd.ClipDictionary
            ?? throw new InvalidDataException("YCD has no clip dictionary.");

        if (clipDict.ClipMap is { Count: > 0 })
        {
            ClipMapEntry? matchedEntry = null;
            MetaHash matchedKey = default;
            bool matched = false;
            foreach (var kv in clipDict.ClipMap)
            {
                if (!ClipNameMatches(kv.Key, kv.Value, clipName)) continue;
                matched = true;
                matchedKey = kv.Key;
                matchedEntry = kv.Value;
                var resolved = ResolveClipMapEntry(clipDict, kv.Key, kv.Value);
                if (resolved is not null)
                    return BuildClip(clipDict, resolved.Value.name, resolved.Value.anim, kv.Value.Clip, rigBoneNames);
            }

            // Name matched but CW didn't wire Animation — chase AnimMap by
            // basename / any single animation in the dict.
            if (matched && matchedEntry is not null)
            {
                var loose = ResolveAnimationLoose(clipDict, matchedKey, matchedEntry, clipName);
                if (loose is not null)
                    return BuildClip(clipDict, ClipListName(matchedKey, matchedEntry), loose, matchedEntry.Clip, rigBoneNames);
            }

            throw new InvalidDataException(matched
                ? "Couldn't resolve animation for clip: " + clipName
                : "Clip not found: " + clipName);
        }

        object? targetClip = null;
        var clips = EnumerateClips(clipDict);
        if (clips is not null)
        {
            foreach (var c in clips)
            {
                var n = ReadString(c, "Name") ?? ReadString(c, "ShortName");
                if (NamesMatch(n, clipName))
                {
                    targetClip = c;
                    break;
                }
            }
        }
        targetClip ??= FindFirstClipObject(clipDict)
            ?? throw new InvalidDataException("Clip not found: " + clipName);

        var named = ResolveClipObject(clipDict, targetClip)
            ?? throw new InvalidDataException("Couldn't resolve clip's animation in dictionary.");
        return BuildClip(clipDict, named.name, named.anim, targetClip, rigBoneNames);
    }

    /// <summary>Stable UI name: ShortName → basename(Name) → hash string.</summary>
    private static string ClipListName(MetaHash key, ClipMapEntry entry)
    {
        var clip = entry.Clip;
        if (clip is not null)
        {
            if (!string.IsNullOrWhiteSpace(clip.ShortName)
                && !LooksLikeBareHash(clip.ShortName))
                return clip.ShortName.Trim();
            var baseName = ClipBasename(clip.Name);
            if (!string.IsNullOrWhiteSpace(baseName) && !LooksLikeBareHash(baseName))
                return baseName;
            if (!string.IsNullOrWhiteSpace(clip.Name))
                return clip.Name.Trim();
        }
        var keyStr = key.ToString();
        return ClipBasename(keyStr) ?? keyStr;
    }

    private static bool ClipNameMatches(MetaHash key, ClipMapEntry entry, string clipName)
    {
        if (string.IsNullOrWhiteSpace(clipName)) return false;
        if (NamesMatch(key.ToString(), clipName)) return true;
        if (NamesMatch(ClipListName(key, entry), clipName)) return true;
        if (NamesMatch(ClipLabel(key, entry), clipName)) return true;
        var clip = entry.Clip;
        if (clip is null) return false;
        return NamesMatch(clip.Name, clipName)
            || NamesMatch(clip.ShortName, clipName)
            || NamesMatch(ClipBasename(clip.Name), clipName)
            || NamesMatch(ClipBasename(clip.ShortName), clipName);
    }

    private static bool NamesMatch(string? a, string? b)
    {
        if (string.IsNullOrWhiteSpace(a) || string.IsNullOrWhiteSpace(b)) return false;
        if (string.Equals(a.Trim(), b.Trim(), StringComparison.OrdinalIgnoreCase)) return true;
        var ba = ClipBasename(a);
        var bb = ClipBasename(b);
        return ba is not null && bb is not null
            && string.Equals(ba, bb, StringComparison.OrdinalIgnoreCase);
    }

    private static string? ClipBasename(string? name)
    {
        if (string.IsNullOrWhiteSpace(name)) return null;
        var s = name.Trim();
        // pack:/foo/bar.clip  or  foo\\bar.clip
        int slash = s.LastIndexOfAny(['/', '\\']);
        if (slash >= 0 && slash < s.Length - 1)
            s = s[(slash + 1)..];
        if (s.EndsWith(".clip", StringComparison.OrdinalIgnoreCase))
            s = s[..^5];
        return string.IsNullOrWhiteSpace(s) ? null : s;
    }

    private static bool LooksLikeBareHash(string? s)
    {
        if (string.IsNullOrWhiteSpace(s)) return true;
        var t = s.Trim();
        if (t.StartsWith("0x", StringComparison.OrdinalIgnoreCase))
            return t.Length >= 6 && t.Skip(2).All(static c => char.IsAsciiHexDigit(c));
        return t.Length >= 8 && t.All(char.IsDigit);
    }

    /// <summary>When ClipMap entry has no wired Animation, find one by hash
    /// or by matching AnimMap key / basename to the clip name.</summary>
    private static Animation? ResolveAnimationLoose(
        ClipDictionary clipDict, MetaHash clipHash, ClipMapEntry entry, string clipName)
    {
        var direct = ResolveAnimationForClip(clipDict, clipHash, entry);
        if (direct is Animation a0) return a0;

        if (entry.Clip is ClipAnimation ca)
        {
            if (ca.Animation is not null) return ca.Animation;
            var byHash = FindAnimationInAnimMap(clipDict, ca.AnimationHash);
            if (byHash is not null) return byHash;
        }

        // ClipAnimationList — take the first entry that has an Animation.
        if (entry.Clip is not null)
        {
            var fromList = ResolveFromClipAnimationList(entry.Clip);
            if (fromList is not null) return fromList;
        }

        // Jenkins/joaat of the short name (common when AnimMap key ≠ ClipMap key).
        if (!string.IsNullOrWhiteSpace(clipName))
        {
            var byNameHash = FindAnimationInAnimMap(clipDict, new MetaHash(Joaat(clipName)));
            if (byNameHash is not null) return byNameHash;
            var baseName = ClipBasename(clipName);
            if (baseName is not null && !NamesMatch(baseName, clipName))
            {
                byNameHash = FindAnimationInAnimMap(clipDict, new MetaHash(Joaat(baseName)));
                if (byNameHash is not null) return byNameHash;
            }
        }

        // AnimMap keyed by a string that matches our list name.
        if (clipDict.AnimMap is { Count: > 0 })
        {
            foreach (var kv in clipDict.AnimMap)
            {
                if (kv.Value.Animation is null) continue;
                if (NamesMatch(kv.Key.ToString(), clipName)
                    || NamesMatch(ClipBasename(kv.Key.ToString()), clipName))
                    return kv.Value.Animation;
            }
            // Single-anim dictionaries: clip/anim hashes often differ.
            if (clipDict.AnimMap.Count == 1)
            {
                var only = clipDict.AnimMap.First().Value.Animation;
                if (only is not null) return only;
            }
        }

        var anims = EnumerateAnimations(clipDict).ToList();
        if (anims.Count == 1) return anims[0];
        return null;
    }

    private static Animation? TryResolvePlayable(
        ClipDictionary clipDict, MetaHash key, ClipMapEntry entry, string label)
    {
        var resolved = ResolveClipMapEntry(clipDict, key, entry);
        if (resolved is { } r && r.anim is Animation a && AnimationLooksPlayable(a))
            return a;

        var loose = ResolveAnimationLoose(clipDict, key, entry, label);
        if (loose is not null && AnimationLooksPlayable(loose))
            return loose;
        return null;
    }

    private static object? ResolveNamedAnimation(ClipDictionary clipDict, string clipName)
    {
        if (clipDict.ClipMap is { Count: > 0 })
        {
            foreach (var kv in clipDict.ClipMap)
            {
                if (!ClipNameMatches(kv.Key, kv.Value, clipName)) continue;
                var playable = TryResolvePlayable(clipDict, kv.Key, kv.Value, clipName);
                if (playable is not null) return playable;
            }
        }
        return null;
    }

    private static Animation? ResolveFromClipAnimationList(object clip)
    {
        // ClipAnimationList exposes Animations[] of entries with Animation refs.
        var entries = ReadEnumerable(clip, "Animations");
        if (entries is null) return null;
        foreach (var item in entries)
        {
            if (item is null) continue;
            if (ReadValue(item, "Animation") is Animation anim)
                return anim;
        }
        return null;
    }

    private static bool AnimationLooksPlayable(object? animation)
    {
        if (animation is not Animation anim) return animation is not null;
        try
        {
            anim.AssignSequenceBoneIds();
            var ids = anim.BoneIds?.data_items;
            var seqs = anim.Sequences?.data_items;
            return ids is { Length: > 0 } && seqs is { Length: > 0 };
        }
        catch
        {
            return false;
        }
    }

    // ── Internal: walk the dictionary structure ─────────────────────

    /// <summary>Find the first clip + animation pair using every layout
    /// shipping .ycd files use (ClipMap, AnimMap, sparse Clips[], Animations block).</summary>
    private static (string name, object anim, object? source)? ResolveFirstClipAndAnimation(ClipDictionary clipDict)
    {
        if (clipDict.ClipMap is { Count: > 0 })
        {
            foreach (var kv in clipDict.ClipMap)
            {
                var resolved = ResolveClipMapEntry(clipDict, kv.Key, kv.Value);
                if (resolved is not null) return (resolved.Value.name, resolved.Value.anim, kv.Value);
            }
        }

        foreach (var clip in EnumerateClips(clipDict) ?? Enumerable.Empty<object>())
        {
            var resolved = ResolveClipObject(clipDict, clip);
            if (resolved is not null) return (resolved.Value.name, resolved.Value.anim, clip);
        }

        // Clip/anim hashes often differ (dict name vs internal anim name) — pair
        // when there is exactly one of each, which covers most dpemotes ycds.
        if (clipDict.ClipMap is { Count: 1 } && clipDict.AnimMap is { Count: 1 })
        {
            var clipKv = clipDict.ClipMap.First();
            var animKv = clipDict.AnimMap.First();
            if (animKv.Value.Animation is not null)
                return (ClipLabel(clipKv.Key, clipKv.Value), animKv.Value.Animation, clipKv.Value);
        }

        foreach (var anim in EnumerateAnimations(clipDict))
            return (anim.Hash.ToString(), anim, null);

        if (clipDict.AnimMap is { Count: > 0 })
        {
            foreach (var kv in clipDict.AnimMap)
            {
                if (kv.Value.Animation is not null)
                    return (kv.Key.ToString(), kv.Value.Animation, null);
            }
        }

        return null;
    }

    private static IEnumerable<object>? EnumerateClips(ClipDictionary clipDict)
        => EnumerateCollection(clipDict.Clips);

    private static IEnumerable<Animation> EnumerateAnimations(ClipDictionary clipDict)
    {
        var block = clipDict.Animations?.Animations;
        if (block is null) yield break;

        if (block.data_items is { Length: > 0 } items)
        {
            foreach (var item in items)
            {
                var anim = UnwrapAnimation(item);
                if (anim is not null) yield return anim;
            }
            yield break;
        }

        for (int i = 0; i < block.Count; i++)
        {
            var anim = UnwrapAnimation(block[i]);
            if (anim is not null) yield return anim;
        }
    }

    private static Animation? UnwrapAnimation(object? item)
    {
        if (item is Animation anim) return anim;
        if (item is AnimationMapEntry entry) return entry.Animation;
        return null;
    }

    private static (string name, object anim)? ResolveClipMapEntry(
        ClipDictionary clipDict, MetaHash clipHash, ClipMapEntry entry)
    {
        var anim = ResolveAnimationForClip(clipDict, clipHash, entry);
        if (anim is null) return null;
        return (ClipLabel(clipHash, entry), anim);
    }

    private static string ClipLabel(MetaHash clipHash, ClipMapEntry entry)
        => entry.Clip?.Name ?? entry.Clip?.ShortName ?? clipHash.ToString();

    private static (string name, object anim)? ResolveClipObject(ClipDictionary clipDict, object clip)
    {
        if (clip is ClipMapEntry mapEntry)
            return ResolveClipMapEntry(clipDict, mapEntry.Hash, mapEntry);

        if (clip is ClipAnimation clipAnim)
        {
            var anim = clipAnim.Animation ?? FindAnimationInAnimMap(clipDict, clipAnim.AnimationHash);
            if (anim is not null)
                return (clipAnim.Name ?? clipAnim.ShortName ?? clipAnim.Hash.ToString(), anim);
        }

        return ResolveClipAnimation(clipDict, clip);
    }

    private static object? ResolveAnimationForClip(ClipDictionary clipDict, MetaHash clipHash, ClipMapEntry entry)
    {
        if (clipDict.AnimMap.TryGetValue(clipHash, out var byClipKey) && byClipKey.Animation is not null)
            return byClipKey.Animation;

        if (entry.Clip is ClipAnimation clipAnim)
        {
            if (clipAnim.Animation is not null) return clipAnim.Animation;
            var byAnimHash = FindAnimationInAnimMap(clipDict, clipAnim.AnimationHash);
            if (byAnimHash is not null) return byAnimHash;
        }

        var byEntryHash = FindAnimationInAnimMap(clipDict, entry.Hash);
        if (byEntryHash is not null) return byEntryHash;

        return FindAnimationInAnimMap(clipDict, clipHash);
    }

    private static Animation? FindAnimationInAnimMap(ClipDictionary clipDict, MetaHash hash)
    {
        if (clipDict.AnimMap.TryGetValue(hash, out var direct) && direct.Animation is not null)
            return direct.Animation;

        uint h = hash.Hash;
        foreach (var kv in clipDict.AnimMap)
        {
            if (kv.Key.Hash != h) continue;
            if (kv.Value.Animation is not null) return kv.Value.Animation;
        }

        foreach (var anim in EnumerateAnimations(clipDict))
        {
            if (anim.Hash.Hash == h) return anim;
        }

        return null;
    }

    private static object? FindFirstClipObject(ClipDictionary clipDict)
    {
        if (clipDict.ClipMap is { Count: > 0 })
            return clipDict.ClipMap.First().Value;

        return EnumerateClips(clipDict)?.FirstOrDefault(c => c is not null);
    }

    private static (string name, object anim)? ResolveClipAnimation(ClipDictionary clipDict, object clip)
    {
        if (clip is ClipMapEntry mapEntry)
            return ResolveClipMapEntry(clipDict, mapEntry.Hash, mapEntry);

        var name = ReadString(clip, "Name") ?? ReadString(clip, "ShortName") ?? "imported";

        // ClipAnimation exposes the resolved Animation reference directly —
        // CW already chased the AnimationHash through the dictionary at load
        // time, so we don't need to walk it again.
        var direct = ReadValue(clip, "Animation");
        if (direct is not null) return (name, direct);

        // ClipAnimationList carries multiple ClipAnimationsEntry items, each
        // pointing at its own Animation. Take the first for the importer's
        // "first clip" semantics; the multi-clip picker can extend this.
        var entries = ReadEnumerable(clip, "Animations");
        var firstEntry = entries?.FirstOrDefault();
        if (firstEntry is not null)
        {
            var anim = ReadValue(firstEntry, "Animation");
            if (anim is not null) return (name, anim);
        }

        // Last-ditch fallback: look the clip's animation hash up in the
        // dictionary's AnimMap (Dictionary<MetaHash, AnimationMapEntry>).
        // This path matters only if CW didn't auto-resolve the pointer,
        // which shouldn't happen on Load — kept for forward-compat.
        var hashObj = ReadValue(clip, "AnimationHash") ?? ReadValue(clip, "NameHash") ?? ReadValue(clip, "Hash");
        if (hashObj is MetaHash mh)
        {
            var anim = FindAnimationInAnimMap(clipDict, mh);
            if (anim is not null) return (name, anim);
        }
        return null;
    }

    /// <summary>A clip's playable slice of its source animation: seconds
    /// [Start,End] played at Rate. GTA "move" clips are short windows into
    /// 40+s shared animations, so importing the whole animation gave clips
    /// the wrong length AND wrong content.</summary>
    private readonly record struct ClipWindow(float Start, float End, float Rate);

    /// <summary>Every playable segment of a clip, each pairing its OWN source
    /// animation with its OWN [Start,End,Rate] window. A GTA turn clip is a
    /// ClipAnimationList whose segments split the skeleton — the legs live in
    /// the mover segment, the upper body / overlay in another. Resolving one
    /// animation (via AnimMap) and one window (from a segment) SEPARATELY let
    /// them mismatch: the mover moved but the legs came out frozen → the ped
    /// slid rigidly across the floor. Keeping anim+window together per segment
    /// and compositing them all is the fix.</summary>
    private static List<(Animation anim, ClipWindow win)> ExtractClipSegments(
        ClipDictionary clipDict, object? clip, Animation? fallback)
    {
        var segs = new List<(Animation, ClipWindow)>();
        static ClipWindow W(float s, float e, float rate) =>
            new ClipWindow(s, e, rate > 1e-4f ? rate : 1f);

        switch (clip)
        {
            case ClipMapEntry me:
                return ExtractClipSegments(clipDict, me.Clip, fallback);
            case ClipAnimation ca:
            {
                var a = ca.Animation ?? FindAnimationInAnimMap(clipDict, ca.AnimationHash);
                if (a is not null && ca.EndTime > ca.StartTime + 1e-4f)
                    segs.Add((a, W(ca.StartTime, ca.EndTime, ca.Rate)));
                break;
            }
            case ClipAnimationList cal when cal.Animations is not null:
            {
                foreach (var e in cal.Animations)
                {
                    if (e is null || !(e.EndTime > e.StartTime + 1e-4f)) continue;
                    var a = e.Animation ?? FindAnimationInAnimMap(clipDict, e.AnimationHash);
                    if (a is not null) segs.Add((a, W(e.StartTime, e.EndTime, e.Rate)));
                }
                break;
            }
        }
        if (segs.Count == 0 && fallback is not null)
        {
            float d = fallback.Duration;
            segs.Add((fallback, new ClipWindow(0, d > 0.001f ? d : 1f, 1f)));
        }
        return segs;
    }

    /// <summary>Resolve a clip to its segments and composite them; falls back
    /// to the reflective path when the resolved object isn't a CW Animation.</summary>
    private static ImportResult BuildClip(
        ClipDictionary clipDict, string name, object anim, object? clipObj, IReadOnlyList<string> rig)
    {
        var segs = ExtractClipSegments(clipDict, clipObj, anim as Animation);
        return segs.Count > 0
            ? BuildFromSegments(name, segs, rig)
            : BuildFromAnimation(name, anim, rig);
    }

    private static ImportResult BuildFromAnimation(
        string clipName, object animation, IReadOnlyList<string> rigBoneNames, ClipWindow? window = null)
    {
        if (animation is Animation cwAnim)
            return BuildFromCodewalkerAnimation(clipName, cwAnim, rigBoneNames, window);

        // Legacy reflection path — kept for forward-compat if CW's in-memory
        // model changes type names but the property layout stays similar.
        return BuildFromAnimationReflective(clipName, animation, rigBoneNames);
    }

    /// <summary>Single-animation import — a one-segment composite.</summary>
    private static ImportResult BuildFromCodewalkerAnimation(
        string clipName, Animation anim, IReadOnlyList<string> rigBoneNames, ClipWindow? window = null)
    {
        anim.AssignSequenceBoneIds();
        float dur = anim.Duration;
        var win = window ?? new ClipWindow(0f, dur > 0.001f ? dur : 1f, 1f);
        return BuildFromSegments(clipName, new List<(Animation, ClipWindow)> { (anim, win) }, rigBoneNames);
    }

    /// <summary>Composite a clip's segments onto one preview timeline. The
    /// primary segment (carrying the mover, else the richest) sets fps + clip
    /// length; every segment maps its own window across that shared timeline
    /// and writes its bone tracks. GTA turn clips split legs into the mover
    /// segment and the upper body into an overlay — importing one alone froze
    /// half the skeleton and skated.</summary>
    private static ImportResult BuildFromSegments(
        string clipName, List<(Animation anim, ClipWindow win)> segments, IReadOnlyList<string> rigBoneNames)
    {
        if (segments.Count == 0)
            throw new InvalidDataException("Clip has no playable animation segments.");

        var tagToRigName = BuildTagToRigNameMap(rigBoneNames);
        var rotationTracksByTag = new Dictionary<ushort, Quaternion[]>();
        var positionTracksByTag = new Dictionary<ushort, Vector3[]>();
        Vector3[]? moverPosition = null;
        Quaternion[]? moverRotation = null;
        int unmapped = 0;

        // Primary segment: the one carrying the mover (Track 5), else the one
        // with the most tracks. It defines fps + clip length.
        int primary = 0, bestScore = int.MinValue;
        for (int s = 0; s < segments.Count; s++)
        {
            var a = segments[s].anim;
            a.AssignSequenceBoneIds();
            var ids = a.BoneIds?.data_items;
            int score = (ids != null && ids.Any(x => x.Track == 5) ? 100000 : 0) + (ids?.Length ?? 0);
            if (score > bestScore) { bestScore = score; primary = s; }
        }

        var pAnim = segments[primary].anim;
        var pWin = segments[primary].win;
        float pDur = pAnim.Duration;
        int pFrames = pAnim.Frames;
        int fps = (pDur > 0.001f && pFrames > 1) ? (int)Math.Round((pFrames - 1) / pDur) : 30;
        fps = Math.Clamp(fps, 1, 120);
        if (pFrames < 2 && pDur > 0.001f) pFrames = Math.Max(2, (int)Math.Round(pDur * fps) + 1);

        float pStart = pDur > 0.001f ? Math.Clamp(pWin.Start, 0f, pDur) : pWin.Start;
        float pEnd = pDur > 0.001f ? Math.Clamp(pWin.End, pStart, pDur) : pWin.End;
        float pSpan = Math.Max(1e-3f, pEnd - pStart);
        float clipDuration = pSpan / (pWin.Rate > 1e-4f ? pWin.Rate : 1f);
        int previewFrames = ComputePreviewFrameCount(Math.Max(2, pFrames), fps, clipDuration);

        // Sample the PRIMARY first so its mover wins; overlays layer their
        // bone tracks on top (disjoint bone sets in practice, so last-wins is
        // safe). All segments share `previewFrames` — each maps its window
        // linearly across the clip timeline.
        SampleSegmentInto(segments[primary].anim, segments[primary].win, previewFrames,
            tagToRigName, rotationTracksByTag, positionTracksByTag,
            ref moverPosition, ref moverRotation, ref unmapped);
        for (int s = 0; s < segments.Count; s++)
        {
            if (s == primary) continue;
            SampleSegmentInto(segments[s].anim, segments[s].win, previewFrames,
                tagToRigName, rotationTracksByTag, positionTracksByTag,
                ref moverPosition, ref moverRotation, ref unmapped);
        }

        return FinishImportResult(clipName, previewFrames, fps, clipDuration, tagToRigName,
            rotationTracksByTag, positionTracksByTag, unmapped, moverPosition, moverRotation);
    }

    /// <summary>Read tracks the way CodeWalker does (BoneIds[i] pairs with
    /// Sequences[chunk].Sequences[i]) for ONE segment, sampling only its
    /// [Start,End] window across the shared preview timeline. Mover tracks
    /// (5/6) fill only if not already set (primary wins); bone tracks (0/1)
    /// last-write-wins so overlays layer on top.</summary>
    private static void SampleSegmentInto(
        Animation anim, ClipWindow win, int previewFrames,
        Dictionary<ushort, string> tagToRigName,
        Dictionary<ushort, Quaternion[]> rotationTracksByTag,
        Dictionary<ushort, Vector3[]> positionTracksByTag,
        ref Vector3[]? moverPosition, ref Quaternion[]? moverRotation, ref int unmapped)
    {
        anim.AssignSequenceBoneIds();
        float dur = anim.Duration;
        float winStart = dur > 0.001f ? Math.Clamp(win.Start, 0f, dur) : Math.Max(0f, win.Start);
        float winEnd = dur > 0.001f ? Math.Clamp(win.End, winStart, dur) : win.End;
        float winSpan = Math.Max(1e-3f, winEnd - winStart);
        float SampleTime(int f) => previewFrames > 1
            ? winStart + (f / (float)(previewFrames - 1)) * winSpan
            : winStart;

        var boneIds = anim.BoneIds?.data_items;
        if (boneIds is not { Length: > 0 } || anim.Sequences?.data_items is not { Length: > 0 }) return;

        for (int i = 0; i < boneIds.Length; i++)
        {
            var item = boneIds[i];
            ushort boneTag = item.BoneId;
            byte track = item.Track;

            if (track == 5)
            {
                if (moverPosition != null) continue;   // primary's mover already set
                var pf = new Vector3[previewFrames];
                for (int f = 0; f < previewFrames; f++)
                {
                    var fp = anim.GetFramePosition(SampleTime(f));
                    try { var v = anim.EvaluateVector4(fp, i, true); pf[f] = new Vector3(v.X, v.Y, v.Z); }
                    catch { pf[f] = Vector3.Zero; }
                }
                moverPosition = pf;
                continue;
            }
            if (track == 6)
            {
                if (moverRotation != null) continue;
                var pf = new Quaternion[previewFrames];
                for (int f = 0; f < previewFrames; f++)
                {
                    var fp = anim.GetFramePosition(SampleTime(f));
                    try { var q = anim.EvaluateQuaternion(fp, i, true); pf[f] = new Quaternion(q.X, q.Y, q.Z, q.W); }
                    catch { pf[f] = Quaternion.Identity; }
                }
                moverRotation = pf;
                continue;
            }

            if (!tagToRigName.ContainsKey(boneTag)) { if (track is 0 or 1) unmapped++; continue; }

            if (track == 1)
            {
                var pf = new Quaternion[previewFrames];
                for (int f = 0; f < previewFrames; f++)
                {
                    var fp = anim.GetFramePosition(SampleTime(f));
                    try { var q = anim.EvaluateQuaternion(fp, i, true); pf[f] = new Quaternion(q.X, q.Y, q.Z, q.W); }
                    catch { pf[f] = Quaternion.Identity; }
                }
                rotationTracksByTag[boneTag] = pf;
            }
            else if (track == 0)
            {
                var pf = new Vector3[previewFrames];
                for (int f = 0; f < previewFrames; f++)
                {
                    var fp = anim.GetFramePosition(SampleTime(f));
                    try { var v = anim.EvaluateVector4(fp, i, true); pf[f] = new Vector3(v.X, v.Y, v.Z); }
                    catch { pf[f] = Vector3.Zero; }
                }
                positionTracksByTag[boneTag] = pf;
            }
        }
    }

    private static ImportResult BuildFromAnimationReflective(
        string clipName, object animation, IReadOnlyList<string> rigBoneNames)
    {
        var frameCount = ReadInt(animation, "Frames") ?? ReadInt(animation, "FrameCount") ?? 0;
        var duration   = ReadFloat(animation, "Duration") ?? 1.0f;
        int fps = (duration > 0.001f && frameCount > 1)
            ? (int)Math.Round((frameCount - 1) / duration)
            : 30;
        fps = Math.Clamp(fps, 1, 120);

        if (frameCount < 2 && duration > 0.001f)
            frameCount = Math.Max(2, (int)Math.Round(duration * fps) + 1);

        const int maxFrames = 12_000;
        if (frameCount > maxFrames)
            frameCount = maxFrames;

        var tagToRigName = BuildTagToRigNameMap(rigBoneNames);
        var rotationTracksByTag = new Dictionary<ushort, Quaternion[]>();
        int unmapped = 0;

        // Pair Animation.BoneIds[i] with SequenceData[i] inside the first chunk.
        var boneIdItems = ReadEnumerable(animation, "BoneIds")?.ToList();
        var chunks = ReadEnumerable(animation, "Sequences");
        var firstChunk = chunks?.FirstOrDefault();
        var animSequences = firstChunk is null
            ? null
            : ReadEnumerable(firstChunk, "Sequences")?.ToList();

        if (boneIdItems is not null && animSequences is not null)
        {
            int pairCount = Math.Min(boneIdItems.Count, animSequences.Count);
            for (int i = 0; i < pairCount; i++)
            {
                var boneIdEntry = boneIdItems[i];
                var boneTag = (ushort)(ReadInt(boneIdEntry, "BoneId") ?? 0);
                var trackKind = ReadInt(boneIdEntry, "Track") ?? -1;
                if (trackKind != 1) continue;
                if (!tagToRigName.ContainsKey(boneTag)) { unmapped++; continue; }

                var channels = ReadEnumerable(animSequences[i], "Channels")?.ToList();
                if (channels is null || channels.Count == 0) continue;
                var perFrame = EvaluateRotationChannels(channels, frameCount);
                if (perFrame is null || perFrame.Length == 0) continue;
                rotationTracksByTag[boneTag] = perFrame;
            }
        }

        return FinishImportResult(clipName,
            ComputePreviewFrameCount(frameCount, fps, duration), fps, duration, tagToRigName,
            rotationTracksByTag, new Dictionary<ushort, Vector3[]>(), unmapped, moverPosition: null);
    }

    private static Dictionary<ushort, string> BuildTagToRigNameMap(IReadOnlyList<string> rigBoneNames)
    {
        var tagToRigName = new Dictionary<ushort, string>();
        foreach (var rn in rigBoneNames)
        {
            if (GtaBoneTags.TryResolve(rn, out var tag) && !tagToRigName.ContainsKey(tag))
                tagToRigName[tag] = rn;
        }
        return tagToRigName;
    }

    /// <summary>RAGE Z-up world → three.js Y-up. Matches the viewer’s
    /// freemode bake: (x, y, z)_zup → (x, z, −y)_yup.</summary>
    private static Vector3 RageZUpToYUp(Vector3 p) => new(p.X, p.Z, -p.Y);

    /// <summary>Build per-frame Y-up travel deltas from Track 5 mover, or
    /// fall back to SKEL_ROOT Track 0 when the pack baked locomotion there.
    /// <paramref name="stripRootBoneP"/> is true when travel was lifted from
    /// bone-local root.p (caller must omit that .p from bone tracks).</summary>
    private static List<float[]>? BuildRootTravelDeltas(
        int previewFrames,
        bool isNative,
        Vector3[]? moverPosition,
        Dictionary<ushort, Vector3[]> positionTracksByTag,
        out bool stripRootBoneP)
    {
        stripRootBoneP = false;
        Vector3[]? src = null;
        bool fromBoneLocal = false;
        if (moverPosition is { Length: > 0 })
        {
            src = moverPosition;
        }
        else if (positionTracksByTag.TryGetValue(0, out var rootPos) && rootPos.Length > 1)
        {
            src = rootPos;
            fromBoneLocal = true;
        }
        if (src is null) return null;

        var converted = new Vector3[previewFrames];
        for (int f = 0; f < previewFrames; f++)
        {
            int srcF = previewFrames <= 1 ? 0
                : (int)Math.Round(f * (src.Length - 1) / (double)(previewFrames - 1));
            srcF = Math.Clamp(srcF, 0, src.Length - 1);
            var p = src[srcF];
            converted[f] = isNative ? RageZUpToYUp(p) : p;
        }
        var p0 = converted[0];
        float maxTravel = 0f;
        var root = new List<float[]>(previewFrames);
        for (int f = 0; f < previewFrames; f++)
        {
            var d = converted[f] - p0;
            maxTravel = Math.Max(maxTravel, MathF.Sqrt(d.X * d.X + d.Z * d.Z));
            root.Add(new[] { d.X, d.Y, d.Z });
        }
        if (maxTravel < 0.001f) return null;
        stripRootBoneP = fromBoneLocal;
        return root;
    }

    /// <summary>Frame-0-relative mover heading in VIEWER convention (radians
    /// about the Y-up axis), unwrapped for continuity so the viewer can lerp
    /// plainly. Track 6 is a pure rotation about RAGE +Z (up); the position
    /// axis map P:(x,y,z)→(x,z,-y) is a proper rotation sending RAGE +Z →
    /// viewer +Y, so the SAME-sign angle applies about +Y (verified numerically
    /// against travel-path curvature on walk_turn_l4 — a negation here turned
    /// the body the wrong way and made turns skate WORSE). Null when the clip
    /// never turns more than ~2° — straight clips shouldn't pay the payload.
    /// LONG-WAY GUARD: Track 6 keyframes are sparse and CW's quaternion slerp
    /// winds the long way between them, so a 90° walk-turn evaluated as a smooth
    /// 273° spin (= 360−87). GTA single-clip locomotion turns are always ≤180°,
    /// so if the net winding exceeds a half-turn it's the slerp artifact — the
    /// whole (shape-preserving) ramp is rescaled to the short-way endpoint.</summary>
    private static List<float>? BuildRootYawDeltas(int previewFrames, Quaternion[]? moverRotation)
    {
        if (moverRotation is not { Length: > 0 }) return null;
        var yaw = new float[previewFrames];
        for (int f = 0; f < previewFrames; f++)
        {
            int srcF = previewFrames <= 1 ? 0
                : (int)Math.Round(f * (moverRotation.Length - 1) / (double)(previewFrames - 1));
            srcF = Math.Clamp(srcF, 0, moverRotation.Length - 1);
            var q = moverRotation[srcF];
            if (q.LengthSquared() < 1e-6f) { yaw[f] = f > 0 ? yaw[f - 1] : 0f; continue; }
            q = Quaternion.Normalize(q);
            var fwd = Vector3.Transform(new Vector3(0, 1, 0), q);
            yaw[f] = MathF.Atan2(-fwd.X, fwd.Y);
        }
        float y0 = yaw[0];
        float prev = 0f;
        var raw = new float[previewFrames];
        for (int f = 0; f < previewFrames; f++)
        {
            float d = yaw[f] - y0;
            while (d - prev > MathF.PI) d -= 2 * MathF.PI;
            while (d - prev < -MathF.PI) d += 2 * MathF.PI;
            prev = d;
            raw[f] = d;
        }
        // Long-way guard (see summary). Rescale the ramp to end at the short-way
        // heading so the body follows the intended ≤180° turn, not a full spin.
        float rawEnd = raw[previewFrames - 1];
        if (MathF.Abs(rawEnd) > MathF.PI && MathF.Abs(rawEnd) > 1e-4f)
        {
            float shortEnd = rawEnd;
            while (shortEnd > MathF.PI) shortEnd -= 2 * MathF.PI;
            while (shortEnd < -MathF.PI) shortEnd += 2 * MathF.PI;
            float scale = shortEnd / rawEnd;
            for (int f = 0; f < previewFrames; f++) raw[f] *= scale;
        }
        float maxAbs = 0f;
        var list = new List<float>(previewFrames);
        for (int f = 0; f < previewFrames; f++)
        {
            maxAbs = Math.Max(maxAbs, MathF.Abs(raw[f]));
            list.Add((float)Math.Round(raw[f], 4));
        }
        return maxAbs < 0.035f ? null : list;
    }

    private static ImportResult FinishImportResult(
        string clipName,
        int previewFrames,
        int sourceFps,
        float duration,
        Dictionary<ushort, string> tagToRigName,
        Dictionary<ushort, Quaternion[]> rotationTracksByTag,
        Dictionary<ushort, Vector3[]> positionTracksByTag,
        int unmapped,
        Vector3[]? moverPosition = null,
        Quaternion[]? moverRotation = null)
    {
        int previewFps = Math.Clamp(Math.Min(sourceFps, PreviewMaxFps), 1, 120);
        double clipDuration = duration > 0.001
            ? duration
            : (previewFrames - 1) / (double)Math.Max(1, previewFps);

        // Ship raw RAGE parent-local absolutes. The viewer converts
        // native → freemode glTF with live poseInitialQuats (Assimp bind
        // maps here disagreed with THREE and face-planted the ped).
        // FiveOS round-trip .ycds skip SKEL_ROOT → LooksLikeNativeRage
        // is false → labeled glb → viewer leaves them alone.
        bool isNative = YcdBoneSpaceConverter.LooksLikeNativeRage(rotationTracksByTag);
        if (isNative)
            FosLogger.Info("ycd", "Import labeled boneSpace=native (viewer will compose bind*q)");

        // Entity mover first — Track 5, else SKEL_ROOT Track 0 travel.
        // When travel is lifted from bone-local root.p, omit that .p from
        // bone tracks so the viewer doesn't double-apply it.
        List<float[]>? root = BuildRootTravelDeltas(
            previewFrames, isNative, moverPosition, positionTracksByTag, out bool stripRootBoneP);
        if (root != null)
            FosLogger.Info("ycd", stripRootBoneP
                ? $"Import SKEL_ROOT.p fallback travel frames={previewFrames}"
                : $"Import mover Track5 travel frames={previewFrames}");

        // Mover heading (Track 6). Without it every turning clip slides:
        // the mover path curves but the body never rotates to follow.
        List<float>? rootYaw = BuildRootYawDeltas(previewFrames, moverRotation);
        if (rootYaw != null)
            FosLogger.Info("ycd",
                $"Import mover Track6 heading frames={previewFrames} turn={rootYaw[^1] * 180.0 / Math.PI:F1}deg");

        var tracks = new List<object>(rotationTracksByTag.Count);
        foreach (var (tag, perFrame) in rotationTracksByTag)
        {
            var name = tagToRigName[tag];
            var qList = new List<float[]>(previewFrames);
            List<float[]>? pList = null;
            if (!(stripRootBoneP && tag == 0)
                && positionTracksByTag.TryGetValue(tag, out var perPos))
            {
                pList = new List<float[]>(previewFrames);
                for (int f = 0; f < previewFrames; f++)
                {
                    int srcF = previewFrames <= 1 ? 0
                        : (int)Math.Round(f * (perPos.Length - 1) / (double)(previewFrames - 1));
                    srcF = Math.Clamp(srcF, 0, perPos.Length - 1);
                    var p = perPos[srcF];
                    pList.Add(new[] { p.X, p.Y, p.Z });
                }
            }
            for (int f = 0; f < previewFrames; f++)
            {
                int srcF = previewFrames <= 1 ? 0
                    : (int)Math.Round(f * (perFrame.Length - 1) / (double)(previewFrames - 1));
                srcF = Math.Clamp(srcF, 0, perFrame.Length - 1);
                var q = perFrame[srcF];
                qList.Add(new[] { q.X, q.Y, q.Z, q.W });
            }
            tracks.Add(pList is null ? new { name, q = qList } : new { name, q = qList, p = pList });
        }

        // Position-only bones (no rotation track in this clip).
        foreach (var (tag, perPos) in positionTracksByTag)
        {
            if (rotationTracksByTag.ContainsKey(tag)) continue;
            if (stripRootBoneP && tag == 0) continue;
            var name = tagToRigName[tag];
            var pList = new List<float[]>(previewFrames);
            for (int f = 0; f < previewFrames; f++)
            {
                int srcF = previewFrames <= 1 ? 0
                    : (int)Math.Round(f * (perPos.Length - 1) / (double)(previewFrames - 1));
                srcF = Math.Clamp(srcF, 0, perPos.Length - 1);
                var p = perPos[srcF];
                pList.Add(new[] { p.X, p.Y, p.Z });
            }
            tracks.Add(new { name, q = new List<float[]>(), p = pList });
        }

        var payload = new
        {
            duration = Math.Round(clipDuration, 3),
            fps = previewFps,
            source = "ycd-import",
            boneSpace = isNative ? "native" : "glb",
            format = "bone-tracks",
            tracks,
            root,
            rootYaw,
        };
        var json = JsonSerializer.Serialize(payload, new JsonSerializerOptions { WriteIndented = false });

        return new ImportResult(
            ClipName: clipName,
            KeyframeCount: previewFrames,
            FrameCount: previewFrames,
            Fps: previewFps,
            Duration: (float)clipDuration,
            MappedBones: rotationTracksByTag.Count,
            UnmappedTracks: unmapped,
            PayloadJson: json);
    }

    /// <summary>Pull per-frame quaternions out of an AnimSequence's
    /// Channels[]. Two shapes appear in shipping YCDs:
    /// (a) one StaticQuaternion channel carrying the whole quat in a
    ///     SharpDX.Quaternion `Value` field — broadcast across all frames;
    /// (b) three float channels (RawFloat / QuantizeFloat / IndirectQuantizeFloat
    ///     / LinearFloat) for qx, qy, qz — W is reconstructed from the
    ///     unit-norm constraint, matching RAGE's runtime behavior.
    /// For (b) we invoke AnimChannel.EvaluateFloat(frame) reflectively so
    /// the same code path handles every compressed encoding without us
    /// having to know which one CW chose for each track.</summary>
    private static Quaternion[]? EvaluateRotationChannels(List<object> channels, int frameCount)
    {
        int safeFrames = Math.Max(1, frameCount);

        // Case A: single StaticQuaternion — Value is a SharpDX.Quaternion
        // struct with X/Y/Z/W fields. We pull the components reflectively
        // so we don't have to take a hard dependency on SharpDX here.
        if (channels.Count == 1)
        {
            var ch = channels[0];
            var typeName = ReadValue(ch, "Type")?.ToString() ?? ch.GetType().Name;
            if (typeName.IndexOf("StaticQuaternion", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                var value = ReadValue(ch, "Value");
                if (value is null) return null;
                var x = ReadFloat(value, "X") ?? 0;
                var y = ReadFloat(value, "Y") ?? 0;
                var z = ReadFloat(value, "Z") ?? 0;
                var w = ReadFloat(value, "W") ?? 1;
                return Enumerable.Repeat(new Quaternion(x, y, z, w), safeFrames).ToArray();
            }
            if (typeName.IndexOf("CachedQuaternion", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                var eval = ch.GetType().GetMethod("EvaluateQuaternion", BindingFlags.Public | BindingFlags.Instance);
                if (eval is not null)
                {
                    var arr = new Quaternion[safeFrames];
                    for (int f = 0; f < safeFrames; f++)
                    {
                        try
                        {
                            var value = eval.Invoke(ch, new object[] { f });
                            if (value is null) continue;
                            arr[f] = new Quaternion(
                                ReadFloat(value, "X") ?? 0,
                                ReadFloat(value, "Y") ?? 0,
                                ReadFloat(value, "Z") ?? 0,
                                ReadFloat(value, "W") ?? 1);
                        }
                        catch { arr[f] = Quaternion.Identity; }
                    }
                    return arr;
                }
            }
            return null;
        }

        // Case B: 3+ float channels (qx, qy, qz[, qw]). Prefer bulk Values[]
        // arrays; fall back to per-frame EvaluateFloat only when needed.
        if (channels.Count >= 3)
        {
            var valsX = GetChannelFrameValues(channels[0], safeFrames);
            var valsY = GetChannelFrameValues(channels[1], safeFrames);
            var valsZ = GetChannelFrameValues(channels[2], safeFrames);
            if (valsX is null || valsY is null || valsZ is null) return null;

            var valsW = channels.Count >= 4 ? GetChannelFrameValues(channels[3], safeFrames) : null;

            var arr = new Quaternion[safeFrames];
            for (int f = 0; f < safeFrames; f++)
            {
                float x = valsX[f], y = valsY[f], z = valsZ[f];
                float w;
                if (valsW is not null)
                    w = valsW[f];
                else
                {
                    float w2 = 1f - (x * x + y * y + z * z);
                    w = w2 > 0 ? MathF.Sqrt(w2) : 0f;
                }
                arr[f] = Quaternion.Normalize(new Quaternion(x, y, z, w));
            }
            return arr;
        }

        return null;
    }

    /// <summary>All non-static float channel types (RawFloat, QuantizeFloat,
    /// IndirectQuantizeFloat, LinearFloat, plus StaticFloat which simply
    /// returns its constant) implement <c>EvaluateFloat(int frame)</c>.
    /// Bind to it reflectively so the importer stays agnostic to which
    /// encoding CW chose for each track.</summary>
    /// <summary>Per-frame float samples for a channel — reads Values[] in
    /// one shot when CW exposes it, otherwise binds EvaluateFloat.</summary>
    private static float[]? GetChannelFrameValues(object channel, int frameCount)
    {
        var values = ReadFloatArray(channel);
        if (values is { Length: > 0 })
        {
            if (values.Length >= frameCount) return values;
            var expanded = new float[frameCount];
            for (int i = 0; i < frameCount; i++)
                expanded[i] = values[Math.Min(i, values.Length - 1)];
            return expanded;
        }

        var eval = GetFloatEvaluator(channel);
        if (eval is null) return null;
        var arr = new float[frameCount];
        for (int f = 0; f < frameCount; f++)
            arr[f] = eval(f);
        return arr;
    }

    private static Func<int, float>? GetFloatEvaluator(object channel)
    {
        var mi = channel.GetType().GetMethod(
            "EvaluateFloat",
            BindingFlags.Public | BindingFlags.Instance,
            binder: null,
            types: new[] { typeof(int) },
            modifiers: null);
        if (mi is not null)
        {
            return frame =>
            {
                try { return Convert.ToSingle(mi.Invoke(channel, new object[] { frame })); }
                catch { return 0f; }
            };
        }
        // Fallback: read the channel's Values[] (RawFloat exposes one).
        var values = ReadFloatArray(channel);
        if (values is null || values.Length == 0) return null;
        return frame => values[Math.Min(frame, values.Length - 1)];
    }

    /// <summary>Load a standalone .ycd resource. The RSC7 header carries the
    /// system/graphics page flags CodeWalker needs to map the resource's memory
    /// blocks — a bare stub entry (name+size only) loads an EMPTY ClipDictionary
    /// (the "no clips" bug). CreateResourceFileEntry parses those flags from the
    /// header and Decompress inflates the payload before Load.</summary>
    private static YcdFile LoadYcd(string path)
        => LoadYcd(File.ReadAllBytes(path), Path.GetFileName(path));

    private static YcdFile LoadYcd(byte[] data, string name)
    {
        ArgumentNullException.ThrowIfNull(data);
        var entry = RpfFile.CreateResourceFileEntry(ref data, 0);
        entry.Name = string.IsNullOrWhiteSpace(name) ? "clip.ycd" : Path.GetFileName(name);
        entry.NameLower = entry.Name.ToLowerInvariant();
        data = ResourceBuilder.Decompress(data);
        var ycd = new YcdFile();
        ycd.Load(data, entry);
        return ycd;
    }

    // ── Reflection helpers ──────────────────────────────────────────

    private static object? ReadValue(object? obj, string name)
    {
        if (obj is null) return null;
        var t = obj.GetType();
        var p = t.GetProperty(name, BindingFlags.Public | BindingFlags.Instance);
        if (p is not null) return p.GetValue(obj);
        var f = t.GetField(name, BindingFlags.Public | BindingFlags.Instance);
        return f?.GetValue(obj);
    }

    private static string? ReadString(object? obj, string name)
        => ReadValue(obj, name)?.ToString();

    private static int? ReadInt(object? obj, string name)
    {
        var v = ReadValue(obj, name);
        if (v is null) return null;
        try { return Convert.ToInt32(v); } catch { return null; }
    }

    private static float? ReadFloat(object? obj, string name)
    {
        var v = ReadValue(obj, name);
        if (v is null) return null;
        try { return Convert.ToSingle(v); } catch { return null; }
    }

    private static IEnumerable<object>? ReadEnumerable(object? obj, string name)
    {
        var v = ReadValue(obj, name);
        return EnumerateCollection(v);
    }

    /// <summary>Walk a CodeWalker collection without calling
    /// <c>ResourcePointerArray64&lt;T&gt;.GetEnumerator()</c> — that API throws
    /// <see cref="NotImplementedException"/> even though <c>Count</c>,
    /// <c>data_items</c>, and the indexer work fine.</summary>
    private static IEnumerable<object>? EnumerateCollection(object? v)
    {
        if (v is null || v is string) return null;

        var t = v.GetType();
        if (t.IsGenericType && t.Name.StartsWith("ResourcePointerArray64", StringComparison.Ordinal))
        {
            if (ReadValue(v, "data_items") is Array items && items.Length > 0)
                return items.Cast<object>().Where(x => x is not null)!;

            int count = ReadInt(v, "Count") ?? 0;
            var itemProp = t.GetProperty("Item");
            if (itemProp is null || count <= 0) return Array.Empty<object>();

            var list = new List<object>(count);
            for (int i = 0; i < count; i++)
            {
                var item = itemProp.GetValue(v, new object[] { i });
                if (item is not null) list.Add(item);
            }
            return list;
        }

        if (v is System.Collections.IList ilist)
            return ilist.Cast<object>().Where(x => x is not null)!;

        if (v is System.Collections.IEnumerable e)
        {
            try { return e.Cast<object>().Where(x => x is not null)!.ToList(); }
            catch (NotImplementedException) { return null; }
        }

        return null;
    }

    private static float[]? ReadFloatArray(object? channel)
    {
        // RawFloat channel's per-frame values are usually exposed as
        // either a Values array (float[]) or an enumerable of frames.
        var v = ReadValue(channel, "Values");
        if (v is float[] arr) return arr;
        if (v is System.Collections.IEnumerable e)
        {
            try { return e.Cast<object>().Select(o => Convert.ToSingle(o)).ToArray(); }
            catch { return null; }
        }
        return null;
    }

    /// <summary>GTA-style lowercase Jenkins one-at-a-time hash (joaat).</summary>
    private static uint Joaat(string s)
    {
        uint hash = 0;
        foreach (var ch in s.ToLowerInvariant())
        {
            hash += ch;
            hash += hash << 10;
            hash ^= hash >> 6;
        }
        hash += hash << 3;
        hash ^= hash >> 11;
        hash += hash << 15;
        return hash;
    }
}
