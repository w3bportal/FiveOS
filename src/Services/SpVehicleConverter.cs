...
public sealed class SpVehicleConverter
{
    ...
    public Result Convert(string inputPath, string outputRoot, Options opts, Action<string> log)
        => Convert(new[] { inputPath }, outputRoot, opts, log);

    public Result Convert(IReadOnlyList<string> inputPaths, string outputRoot, Options opts, Action<string> log)
    {
        ...
        try
        {
            ...
            foreach (var (rel, type) in dataFiles.Distinct())
                fx.AppendLine($"data_file '{type}' '{rel}'");
            ...
            if (displayNames.Count > 0)
            {
                ...
                foreach (var (game, label) in displayNames)
                    lua.AppendLine($"    AddTextEntry('{game}', '{label.Replace("'", "\\'")}')");
                lua.AppendLine("end)");
