...
/// plus meta files) into a single OPEN (unencrypted) RPF7 archive that opens
...
public sealed class RpfPacker
{
    ...
    private static string? ShouldSkip(string name, string ext, Options opts)
    {
        ...
        // A pre-packed child .rpf is stored as a plain binary entry (how the
        // game ships nested archives) — but only when the user opts into
        // Include-all; the default RAGE pass-list keeps archives flat.
        if (string.Equals(ext, ".rpf", StringComparison.OrdinalIgnoreCase) && !opts.IncludeAllFiles)
            return "nested .rpf (enable 'Include all files' to pack it)";
        if (!opts.IncludeAllFiles && !RageExts.Contains(ext))
            return "extension not in the RAGE pack list (enable 'Include all files' to pack it)";
        return null;
    }
