...
///
/// An UPPER-BODY "walking" emote keeps the legs free to run locomotion so the
/// PLAYER can walk with WASD — that's <see cref="Walkable"/>/<see cref="UpperBody"/>
/// (they set the upper-body bit; the clip's own leg/root motion is NOT applied).
///
/// <see cref="RootMotion"/> is the opposite and is what a Video → Emote that
/// travels needs: a FULL-BODY clip whose baked SKEL_ROOT "mover" is extracted
...
public enum EmoteMovement
{
    ...
    /// the player can move. Gestures, waves, salutes, pointing.</summary>
    UpperBody,

    /// <summary>Upper-body, tuned for moving around while it loops (the
    /// dpemotes "EmoteMoving" behaviour).</summary>
    Walkable,

    /// <summary>Full-body with ROOT MOTION extracted — the ped physically
    /// travels along the clip's baked SKEL_ROOT mover (Video → Emote that
    /// walks/moves). Needs the .ycd to carry a SKEL_ROOT position track, which
    /// FiveOS bakes for video emotes; standard emote menus (dpemotes) can't
...
public static class EmoteMovementExtensions
{
    ...
    public static int ToAnimFlag(this EmoteMovement m, bool looping) => m switch
    {
        ...
        // Upper-body overlay: + AF_UPPERBODY(16) + AF_SECONDARY(32).
        EmoteMovement.UpperBody => looping ? 1 + 16 + 32 : 2 + 16 + 32,   // 49 / 50
        // Moving upper-body: the classic dpemotes 51 (1+2+16+32).
        EmoteMovement.Walkable  => looping ? 1 + 2 + 16 + 32 : 2 + 16 + 32, // 51 / 50
    ...
    public static bool ToEmoteMoving(this EmoteMovement m) =>
        m is EmoteMovement.UpperBody or EmoteMovement.Walkable or EmoteMovement.RootMotion;
    ...
    public static string Label(this EmoteMovement m) => m switch
    {
        ...
        EmoteMovement.Walkable   => "walkable (upper body, moving)",
