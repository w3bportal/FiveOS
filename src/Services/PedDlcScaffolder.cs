...
public sealed class PedDlcScaffolder
{
    ...
    // Only .yft can carry a ped: RAGE ped streaming requires the fragment
    // (skeleton/physics). A .ydr is a map/prop drawable — a ped scaffolded
    // around one builds cleanly but never loads in-game, so it is NOT accepted.
    private static readonly string[] ModelExts = { ".yft" };
        ...
        try
        {
            ...
            if (peds.Count == 0)
            {
                ...
                if (cls == Classification.NoPedFound)
                    reasons.Add("No standalone ped model (.yft) matching a peds.meta <Name> was found — nothing to scaffold a dlc.rpf around.");
                                                     ...
                                                     List<string> allDirs, List<string> warnings, bool hasPedsMeta)
    {
        ...
        foreach (var ped in candidates)
        {
            ...
            var strayYdr = allFiles.FirstOrDefault(f =>
                Path.GetFileNameWithoutExtension(f).Equals(ped, StringComparison.OrdinalIgnoreCase)
                && Path.GetExtension(f).Equals(".ydr", StringComparison.OrdinalIgnoreCase));
            if (model == null)
            {
                warnings.Add(strayYdr != null
                    ? $"'{ped}' only has a .ydr — a ped must ship a .yft fragment (a .ydr can't be streamed as a ped model) — skipped."
                    : $"peds.meta lists '{ped}' but no model file ({string.Join("/", ModelExts)}) was found — skipped.");
                continue;
            }
            if (strayYdr != null)
                warnings.Add($"'{ped}' has a stray {Path.GetFileName(strayYdr)} next to its .yft — the .ydr was ignored (peds stream from the .yft).");
