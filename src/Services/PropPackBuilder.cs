...
{
    public sealed record BuildResult(bool Success, string? ResultPath, string? Error, EngineRunner.OutputMode Mode);

    public static BuildResult Build(PropPackSession session)
    {
        ...
        try
        {
            ...
            var mergeResult = RunMergePack(packSafeName, streamDir, mergedYtypPath);
    ...
    private static BuildResult DeliverToServerShared(string resourceDir, string serverFolder)
    {
        ...
        return new BuildResult(true, dstStream, null, EngineRunner.OutputMode.ServerShared);
    }

    // ───────────────────────── helpers ─────────────────────────

    private static string BuildManifest(string safeName, string displayName, string mergedYtypName)
    {
    ...
    /// .ydr's header, not the texture pages).</summary>
    private static (bool Success, string? Error) RunMergePack(
        string packName, string streamDir, string outYtypPath)
    {
        ...
        try
        {
            ...
            psi.ArgumentList.Add("--lod-dist"); psi.ArgumentList.Add("500");
