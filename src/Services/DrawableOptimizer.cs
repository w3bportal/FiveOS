...
public sealed class DrawableOptimizer
{
        ...
        DrawableBase drawable, Options opts, Action<string>? log)
    {
        ...
        // Rigid animated props (FiveOS "Animated prop") must NOT be geometry-
        // decimated. Merged decimation welds every model's vertices by position
        // and reduces across them, so coincident verts where two SEPARATE moving
        // parts meet (hinges/seams) get collapsed across the part boundary and
        // each part's rest geometry is pulled toward its neighbour. The bindings
        // survive, but the distorted rest pose makes the parts tear/deform when
        // the .ycd clip plays — read as "optimize broke the animation". Skip the
        // geometry pass for these (textures still optimize via OptimizeEmbeddedTxd).
        if (IsRigidAnimatedDrawable(drawable, highModels))
        {
            log?.Invoke("  rigged animated prop (Skeleton + all models HasSkin==0) — skipping geometry decimation to keep moving parts intact; textures still optimized");
            return (0, 0, 0, 0);
        }
    ...
    /// <summary>True when the drawable is a rigid animated prop: a Skeleton is
    /// attached AND every High model is rigid-bound to a bone with no skin
    /// (<c>HasSkin == 0</c>) — the exact signature FiveOS's animated-prop builder
    /// produces (one model per moving part, bound by <c>BoneIndex</c>). Merged
    /// geometry decimation welds those separate parts together and distorts their
    /// rest pose, so callers skip it for these. Returns false for static props
    /// (no Skeleton) and for vehicles / skinned peds &amp; clothing (HasSkin == 1),
    /// which keep decimating exactly as before.</summary>
    internal static bool IsRigidAnimatedDrawable(DrawableBase drawable, DrawableModel[] highModels)
    {
        if (drawable?.Skeleton == null) return false;
        if (highModels == null || highModels.Length == 0) return false;
        foreach (var m in highModels)
            if (m != null && m.HasSkin != 0) return false;   // any skinned model → normal decimate
        return true;
    }
