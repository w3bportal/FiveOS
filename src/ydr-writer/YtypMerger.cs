...
public static class YtypMerger
{
    public static int Run(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("usage: ydr-writer merge-pack --pack-name <n> --stream-dir <dir> --out <ytyp> [--lod-dist 500]");
            return 2;
        }
        ...
        foreach (var ydrPath in Directory.EnumerateFiles(streamDir, "*.ydr"))
        {
            try
            {
                ...
                added++;
                sbLog.AppendLine($"  + {assetName}  bbox=({d.BoundingBoxMin.X:F1},{d.BoundingBoxMin.Y:F1},{d.BoundingBoxMin.Z:F1})..({d.BoundingBoxMax.X:F1},{d.BoundingBoxMax.Y:F1},{d.BoundingBoxMax.Z:F1})  r={d.BoundingSphereRadius:F1}");
            }
            catch (Exception ex)
            {
                skipped++;
