...
public static class YbnBuilder
{
        ...
        IReadOnlyDictionary<string, string>? partMaterials = null)
    {
        ...
        // Resolve each mesh to its collision material index. Glass presets
        // (only one in V1, but the helper centralizes the policy) override
        // to GLASS_SHOOT_THROUGH so bullets pass + shatter VFX plays.
        // Emissive presets don't change the surface — they keep the global
        // pick. Standard meshes also keep the global pick. The result is
        // a small palette of unique indices we then dedupe into the BVH's
        // Materials array.
        ...
        IReadOnlyDictionary<string, string>? partMaterials = null)
    {
        ...
        // defaults intact.
        var comp = BuildComposite(scene, materialName, excludeMeshNames, partMaterials);
