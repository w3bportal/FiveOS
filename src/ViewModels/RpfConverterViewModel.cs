...
public partial class RpfConverterViewModel : ObservableObject
{
    ...
    /// OpenIV mods-folder tree; 3 = REPLACE an existing asset → server resource
    /// (FiveM, reliable); 4 = REPLACE → client-side overlay (FiveM, local-only).
    /// (SP car → FiveM moved to its own Vehicles tab.)
    ...
    public bool IsReplaceServer => OutputModeIndex == 3;
    public bool IsReplaceClient => OutputModeIndex == 4;
    ...
    public async Task ConvertAsync()
    {
        ...
        Summary = IsReplaceMode ? "Building replacement…" : IsDlcMode ? "Building dlc.rpf…" : "Packing…";
        RaiseState();

        var input = InputFolder;
        try
        {
            if (IsReplaceMode) await RunReplaceAsync(input, IsReplaceClient);
