...
public partial class TxAdminOptimizeViewModel : ObservableObject
{
    ...
    /// <summary>Backs the Cancel button. Checked between assets and honoured
    /// mid-asset by the optimizer, so a long server-wide sweep stops promptly.
    /// Recreated per <see cref="RunAsync"/>; disposed in its finally.</summary>
    private CancellationTokenSource? _cts;

    /// <summary>Signals an in-flight <see cref="RunAsync"/> to stop after the
    /// current asset. Safe to call when idle.</summary>
    public void RequestCancel()
    {
        _cts?.Cancel();
        Summary = "Cancelling — finishing the current asset…";
    }
    ...
    public async Task RunAsync()
    {
        ...
        await Task.Run(() =>
        {
            for (int i = 0; i < targets.Count; i++)
            {
                ...
                try
                {
                    ...
                    var outcome = _optimizer.Optimize(row.ResolvedPath!, row.Ext, row.MemKind, plan, token);
                ...
                {
                    // Cancel pressed mid-asset — stop cleanly, don't count it as
                    // a failure. The row falls back to "skipped/cancelled".
                    Marshal(() => { row.RowStatus = TxRowStatus.Skipped; row.Status = "cancelled"; });
                    break;
                }
                catch (Exception ex)
                {
        ...
        finally { IsProcessing = false; Progress = 0; _cts?.Dispose(); _cts = null; }  // never leave the run stuck
        ...
        Summary =
            $"{(canceled ? "Cancelled" : "Done")}. {cleared} cleared, {reduced} reduced (still over), {skipped} unchanged, {failed} failed · " +
