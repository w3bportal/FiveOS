...
public partial class OptimizeViewModel : ObservableObject
{
    ...
    public async Task RunAsync()
    {
        ...
        finally { sw.Stop(); IsProcessing = false; Progress = 0; }  // never leave the run stuck
        ...
        Summary =
            $"Done in {FormatDuration(sw.Elapsed)}. {ok} ok, {fail} failed · {FormatBytes(bytesIn)} → {FormatBytes(bytesOut)} ({savedPct:P0} smaller){triPart}{vertPart}{outNote}";
        _setStatus(Summary);
    }
    ...
    private void RunOneYtd(OptimizeQueueItem item, string outDir)
    {
        ...
        try
        {
            ...
            var results = optimizer.Optimize(tempDir, opts,
                onFile: null, progress: null, cancel: CancellationToken.None);
